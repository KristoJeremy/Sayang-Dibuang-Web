...
const insertCrowdfundCard = (crowdfund) => {
  const crowdfundSection = document.querySelector("#crowdfunds");
    ...
    `<div id="crowdfund-${
      crowdfund.id
            ...
            <div>
              <strong>
                ${crowdfund.profile.user.username}
                  <span class="user-point-${crowdfund.user_id}">
        ...
        <div class="card-body">
            ...
            ${
              crowdfund.user_id != loggedInId && !crowdfund.is_accomplished
                ? `<div>
                <a
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body p-4 rounded mb-4">
                              ...
                              <a
                                ...
                                onclick="addPointWhenContacting(${
                                  crowdfund.id
                                }, ${crowdfund.user_id});"
                                target="_blank">
                              ...
                              ${
                                ...
                                target="_blank"
                                onclick="addPointWhenContacting(${crowdfund.id}, ${crowdfund.user_id});"
                              >
                                <div class="flex-shrink-0">
...
const addPointWhenContacting = (crowdfundId, userId) => {
  fetch(`/crowdfundings/add-point-when-contacting/${crowdfundId}`, {
    method: "POST",
    headers: {
    ...
    .then((data) => {
      alert(data.message);
      const contactModalElement = document.querySelector(
        `#contact-modal-${userId}`
      );
...
window.onload = () => {
  ...
  // handle add crowdfund
  // reference: https://dev.to/tochimclaren/django-ajax-form-with-fetch-api-lob
  const createForm = document.querySelector("#create-crowdfund-form");
  createForm.addEventListener("submit", (e) => {
    e.preventDefault();
    formData = new FormData(createForm);
    fetch("/crowdfundings/create/", {
      method: "POST",
      headers: {
        "X-CSRFToken": window.csrftoken,
      },
      ...
      .then((crowdfund) => {
        ...
        // hides modal
        const createModalElement = document.querySelector("#create-modal");
        ...
        for (i = 0; i < userPointText.length; i++) {
          userPointText[i].innerHTML = `(${crowdfund.profile.poin})`;
        }
      })
      .catch((err) => {
        console.log(err);
